...
function SubjectsSelectForm({disciplinas, disciplinasSelecionadas,  setDisciplinasSelecionadas }) {
    ...
    useEffect(() => {
        async function getTurmas(){
            try {
                const response = await api.get(`/turma/disciplina/${disciplinaSelected.id}`);
                ...
                console.error(error);
                toast.error(`Erro ao carregar Turmas da disciplina ${disciplinaSelected.codigo}`);
            }
        }
    ...
    useEffect(() => {
        async function getInitialTurmas() { //Carrega as turmas da primeira disciplina
            if (disciplinas.length > 0) {
                try {
                    const response = await api.get(`/turma/disciplina/${disciplinas[0].id}`);
                    if (response.status === 200) {
                        const turmasSelectArray = response.data.map((turma) => {
                            return {
                                id: turma.id,
                    ...
                    console.error(error);
                    toast.error(`Erro ao carregar Turmas da disciplina ${disciplinas[0].codigo}`);
                }
            }
        }
    ...
    const addTurmaDisciplina = () => {
        if(disciplinasSelecionadas.length>0){
            let i = disciplinasSelecionadas.findIndex((d) => d.disciplina.id == disciplinaSelected.id);
            if(i == -1){
                ...
                let selecionadas =  disciplinasSelecionadas.filter((s) => s.disciplina.id != disciplinaSelected.id);
        ...
        setTurmaSelected(turmasDisponiveis.length>0? turmasDisponiveis[0] : null);
    ...
    return (
        <div className="grid md:grid-cols-3 md:gap-2 items-center">
            ...
            <div>
                {turmaSelected !=null?
                    <Button type="button" onClick={addTurmaDisciplina} disabled={turmaSelected ==null}  >
                        <Plus size={20} />
                    </Button>
                    :
                    <></>
